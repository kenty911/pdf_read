...
export async function uploadPDF(
  ...
  const busy = existing.some((j) => j.status === 'pending' || j.status === 'processing')
  if (busy) return { ok: false, error: '変換中のジョブがあります。完了後に再度お試しください' }
...
export async function retryJob(
  ...
  const userId = await requireUserId()
  const job = await Job.findById(jobId)
  if (!job || job.userId !== userId) return { ok: false, error: 'ジョブが見つかりません' }
  if (job.status !== 'failed') return { ok: false, error: '失敗したジョブのみ再試行できます' }
