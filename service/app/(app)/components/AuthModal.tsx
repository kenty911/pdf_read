...
import { activate, forgotPassword, login, register, resetPassword } from '@/actions/auth'
...
export default function AuthModal({ initialView, onClose, onSuccess }: Props) {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(res.error ?? '認証に失敗しました'); return }
      onSuccess()
    } finally { setLoading(false) }
  }
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(res.error ?? 'エラーが発生しました'); return }
      setCode('')
      setNewPassword('')
      setView('reset')
    } finally { setLoading(false) }
  ...
  const switchTab = (next: 'login' | 'register') => {
    setView(next); setError(''); setPassword(''); setCode(''); setNewPassword('')
  }

  const goToForgot = () => { setView('forgot'); setError(''); setCode(''); setNewPassword('') }
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (e.target === backdropRef.current && (e.key === 'Enter' || e.key === ' ')) onClose() }}
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 relative">
        <button type="button" onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-600" aria-label="閉じる">
        ...
        {view === 'activate' && (
          <CodeInputView
            title="確認コードを入力"
            description={<><span className="font-medium text-gray-700">{email}</span> に6桁のコードを送信しました。</>}
            submitLabel="会員登録を完了"
            code={code} error={error} loading={loading}
            onCodeChange={setCode} onSubmit={handleActivate}
            onResend={() => { setError(''); handleRegister() }}
          />
        )}
        ...
        {view === 'forgot' && (
          <div>
            ...
            <p className="text-sm text-gray-500 mb-5">登録済みのメールアドレスに確認コードを送信します。</p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="forgot-email">メールアドレス</label>
        ...
        {view === 'reset' && (
          <div>
            ...
            <p className="text-sm text-gray-500 mb-5">
              <span className="font-medium text-gray-700">{email}</span> に送信したコードと新しいパスワードを入力してください。
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="reset-code">確認コード</label>
              ...
              {error && <ErrorBox message={error} />}
              <button type="button" onClick={handleReset} disabled={loading || code.length !== 6 || newPassword.length < 8}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-lg transition-colors text-sm">
                {loading ? '処理中...' : 'パスワードを再設定'}
              </button>
              <p className="text-center text-xs text-gray-400">
                コードが届かない場合は{' '}
                <button type="button" onClick={() => { setError(''); handleForgot() }} disabled={loading}
                  className="text-blue-600 hover:underline disabled:opacity-50">再送信</button>
              </p>
            </div>
          </div>
        )}
        ...
        {(view === 'login' || view === 'register') && (
          <>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="auth-email">メールアドレス</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="auth-password">
                  パスワード {view === 'register' && <span className="ml-1 text-xs text-gray-400">（8文字以上）</span>}
                </label>
                <input id="auth-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="••••••••" autoComplete={view === 'register' ? 'new-password' : 'current-password'}
                  onKeyDown={(e) => { if (e.key === 'Enter') view === 'register' ? handleRegister() : handleLogin() }} />
              </div>
              {view === 'login' && (
                <div className="text-right">
                  <button type="button" onClick={goToForgot} className="text-xs text-blue-600 hover:underline">
                    パスワードを忘れた
                  </button>
                </div>
              )}
              {error && <ErrorBox message={error} />}
              <button type="button" onClick={view === 'register' ? handleRegister : handleLogin} disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-lg transition-colors text-sm">
...
function CodeInputView({ title, description, submitLabel, code, error, loading, onCodeChange, onSubmit, onResend }: CodeInputViewProps) {
  return (
    <div>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="auth-code">確認コード</label>
        ...
        <p className="text-center text-xs text-gray-400">
          コードが届かない場合は{' '}
          <button type="button" onClick={onResend} disabled={loading} className="text-blue-600 hover:underline disabled:opacity-50">再送信</button>
        </p>
      </div>
    </div>
  )
